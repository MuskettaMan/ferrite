...
SkydomePipeline::SkydomePipeline(const VulkanBrain& brain, const SwapChain& swapChain, MeshPrimitiveHandle&& sphere, const CameraStructure& camera) :
    _brain(brain),
    ...
    _camera(camera)
{
    ...
    stbi_uc* data = stbi_load("assets/hdri/industrial_sunset_02_puresky_4k.hdr", &width, &height, &numChannels, 4);
...
void SkydomePipeline::RecordCommands(vk::CommandBuffer commandBuffer, uint32_t currentFrame, uint32_t swapChainIndex)
{
